function [Jlinv_so3] = so3_leftInvJacobian(so3)
...
if length(so3) == 3
    ...
    errordlg('Input to so3_leftInvJacobian has incorrect dimensions.','Dimension Error');
    return
end
...
if abs(theta) < 1e-9
    Jlinv_so3 = eye(length(so3)) - wv/2;
else
    alpha = w / theta;
    Jlinv_so3 = 0.5*theta*cot(theta/2)*eye(length(so3)) + ...
