...
for i= 1:sim.numTimeSteps
    % Subtract initial position to ensure that LLA is set as the origin
    [llaEst(1,i),llaEst(2,i),llaEst(3,i)] = ned2geodetic(x_estNED(i,1)-x_estNED(1,1),x_estNED(i,2)-x_estNED(1,2),x_estNED(i,3)-x_estNED(1,3),sim.LLA(1,1),sim.LLA(1,2),sim.LLA(1,3),nav.wgs84);
end
...
plot(sim.time,v_estNED(:,2),'Color','g','DisplayName','x est');
grid minor;
legend;
xlabel('Time [s]');
...
plot(sim.time,v_estNED(:,1),'Color','g','DisplayName','y est');
grid minor;
legend;
xlabel('Time [s]');
ylabel('Linear Velocity [m/s]');
...
plot(sim.time,-v_estNED(:,3),'Color','g','DisplayName','z est');
grid minor;
legend;
xlabel('Time [s]');
ylabel('Linear Velocity [m/s]');
...
% plot(sim.time,a_estNED(:,2),'Color','g','DisplayName','x est');
grid minor;
legend;
xlabel('Time [s]');
...
% plot(sim.time,a_estNED(:,1),'Color','g','DisplayName','y est');
grid minor;
legend;
xlabel('Time [s]');
ylabel('Linear Acceleration [m/s^2]');
...
% plot(sim.time,-a_estNED(:,3),'Color','g','DisplayName','z est');
grid minor;
legend;
xlabel('Time [s]');
ylabel('Linear Acceleration [m/s^2]');
...
for i = 1:size(sim.time,1)
    ...
    sim.s3New(i,:) = quatrotate(qNEW(:,i)',e3);
end
...
plot3(sim.s3New(:,1),sim.s3New(:,2),sim.s3New(:,3),'Color','g','DisplayName','Attitude in Body Frame','LineWidth',2);
