...
% Random initial data
qi = sim.qned(:,1);
xi = [sim.x_ned(1,1) sim.v_ned(1,1) sim.a_ned(1,1) sim.x_ned(1,2) sim.v_ned(1,2) sim.a_ned(1,2) sim.x_ned(1,3) sim.v_ned(1,3) sim.a_ned(1,3) qi' zeros(1,3)]';
Pt = eye(9);
Pa = blkdiag(0.01,0.01,0.01,0.1,0.1,0.1);
q = 100;

settingsCA_KF.useGNSSAltitude = true;
